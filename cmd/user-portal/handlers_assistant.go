...
func (up *UserPortal) handleAssistantSummary(w http.ResponseWriter, userID uuid.UUID, req AssistantChatRequest) {
	...
	messages := []llm.Message{
		{
			Role: "system",
			Content: "Ты — AI-ассистент платформы Recontext. Твоя задача — кратко и структурировано пересказывать содержание аудио/видео записей. " +
				"Отвечай на русском языке. Выдели главные темы, ключевые моменты и выводы. Используй пункты и абзацы для структуры.",
		},
		{
			Role:    "user",
			Content: fmt.Sprintf("Сделай краткое содержание записи «%s».\n\nТранскрипция:\n%s", title, transcript),
		},
	}
...
func (up *UserPortal) handleAssistantDefinitions(w http.ResponseWriter, userID uuid.UUID, req AssistantChatRequest) {
	...
	messages := []llm.Message{
		{
			Role: "system",
			Content: "Ты — AI-ассистент платформы Recontext. Твоя задача — извлекать ключевые термины, определения и понятия из транскрипций записей. " +
				"Отвечай на русском языке. Для каждого термина дай краткое определение в контексте записи. Структурируй как список.",
		},
		{
			Role:    "user",
...
func (up *UserPortal) handleAssistantFindVideos(w http.ResponseWriter, userID uuid.UUID, req AssistantChatRequest) {
	...
	messages := []llm.Message{
		{
			Role: "system",
			Content: "Ты — AI-ассистент платформы Recontext. Отвечай на русском языке. " +
				"Кратко опиши, в каких записях упоминается запрашиваемая тема и в каком контексте.",
		},
		{
			Role: "user",
...
func (up *UserPortal) handleAssistantChat(w http.ResponseWriter, userID uuid.UUID, req AssistantChatRequest) {
	...
	messages := []llm.Message{
		{
			Role: "system",
			Content: "Ты — AI-ассистент платформы Recontext для работы с аудио/видео записями и транскрипциями. " +
