...
func (mp *ManagingPortal) handleRoomFinished(req models.WebhookRequest) error {
	...
	// Trigger composite video generation immediately in parallel with transcription
	// This allows video processing to start right away instead of waiting for all transcriptions
	mp.logger.Infof("🎬 Starting composite video generation in parallel with transcription...")
	go func() {
		postProcessor, err := NewVideoPostProcessor(mp.db, globalNotificationService)
		if err != nil {
			mp.logger.Errorf("❌ Failed to create video post-processor: %v", err)
			return
		}

		if err := postProcessor.ProcessMeetingVideo(room.SID); err != nil {
			mp.logger.Errorf("❌ Composite video generation failed for room %s: %v", room.SID, err)
		} else {
			mp.logger.Infof("✅ Composite video generation completed successfully for room %s", room.SID)
		}
	}()
